const ObjectId = require('mongodb').ObjectID;
const authModels = require('../models/auth-model');
const logger = require('../config/winston');

const createUser = (body, callback) => {
  const query = {};
  query.googleId = body.id;

  logger.info(`createUser - ${JSON.stringify(query)}`);
  authModels.insertOne(query, callback);
};

const findByGoogleId = (id, callback) => {

  const query = {
    googleId: id
  };

  authModels.findOne(query, (err, user) => {
    if (err) {
      logger.error(`Database error findOne: ${JSON.stringify(err)}`);
      return callback(err);
    }
    return callback(null, user);
  });
};

const findById = (id, callback) => {
  const _id = new ObjectId(id);
  authModels.findOne(_id, (err, user) => {
    if (err) {
      logger.error(`Database error findOne: ${JSON.stringify(err)}`);
      return callback(err);
    }
    return callback(null, user);
  });
};

const updateCredits = (id, credits, callback) => {
  const filter = {
    googleId: id
  };

  const update = {
    $set: {
      credits
    }
  };

  const projection = {
    returnOriginal: false
  }

  authModels.findOneAndUpdate(filter, update, projection, (err, user) => {
    if (err) {
      logger.error(`Database error findOneAndUpdate: ${JSON.stringify(err)}`);
      return callback(err);
    }
    return callback(null, user);
  });
};

module.exports = { createUser, findByGoogleId, findById, updateCredits };
